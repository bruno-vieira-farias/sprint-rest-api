...
Já ouvi falar muito que o java é muito complexo, verboso, lento e blá... Não acredito nisso. Acredito que por ser uma poderosa e flexível linguagem de programação(e plataforma) existem muitos caminhos para chegar em uma mesma solução, e o desafio é encontrar o conjunto de ferramentas que podem tornar o nosso trabalho mais àgil, seguro e de fácil manutenção para que possamos nos concentrar no que mais interessa, gerar valor.
...
#### Spring Boot
O Spring boot é uma poderosa ferramenta que automatiza e simplifica toda a infraestrutura necessária para executamos nossa aplicação, reduzindo o tempo e o esforço necessário para que tenhamos uma aplicação completa, segura e estável.
Além de tudo é muito fácil realizar customizações, com pelo aquivo application.properties por exemplo.
No caso do nosso projeto, vamos usar o ORM default do Spring Boot que é o Hibernate, também nos beneficiaremos do servidor de aplicação embutido (TomCat).
...
### Spring JPA
O Spring JPA se baseia nos padrões JPA (Java Persitence API), implementando os recursos que necessitamos para uma integração com um banco de dados relacional. Essa integração é simples, com ótios recursos como o CRUD Repository por exemplo, que facilita e agiliza muito a persistencia e recuperaçao de dados do banco.
Bem, parece se enquandrar perfeitamente no nosso cenário, portanto, vamos usar...

### Spring Initializr
Pra quem acho que não dava para ficar mais fácil... Surpresa!
O Spring Initializr é uma ferramenta que prepara todo o nosso projeto. Você informa algumas variavéis como nome do projeto, builder, linguagem, dependencias dentro outras coisinhas e ele cuida do resto para você, criando toda a estrutura de pastas e aqruivos. Feito isso, é só abrir o projeto em sua IDE e começar a se divertir.
Acertou. Vamos usar.

### Postgress com Docker
Como sabemos, em nosso desafio precisamos persistir os dos dados em um banco de dados relacional, assim, iremos utilizar o postgres rodando em Docker. A decisão de executar em docker é porque este modelo dispensa a necessidade de instalação e configuração do banco de dados localmente atendendo perfeitamente nosso caso de uso. (em ambiente de desenvolvimento)
...
Também precisamos do banco de dados, em apenas dois passos ele estara disponível.
...
Para finalizar o preparo de nosso ambiente temos alguns ajustes no arquivo application.properties.
...
Considerando uma aplicação MVC vamos à implementação do nosso controlller
![](doc/images/MVC.png)

### Camada Controller
Aqui temos nossa primeira classe na camada controller,vamos destacar alguns pontos importantes.
...
- @PostMapping -> Anotação sobre o metodo que será executando sempre que receber uma requisição http POST. 
- @Validated -> Anotaçao sobre os parametros que "diz ao spring" que a validação deve ser realizada. (Bean Validation)
- @RequestBody -> Anotação sobre os parametros que define estes devem ser recebidos via body do http.
- `class ClientDto` -> Classe que será utilizada para bind do JSON recebido na requisição em um objeto Java. 
- @ExceptionHandler -> Anotação sobre o método que captura e trata as exceções `MethodArgumentNotValidException` lançadas.
- HttpStatus -> Determinal qual será o status HTTP retornado.
...
DTO (Data Transfer Object) utilizado para o bind, ou seja, transformação dos dados recebidos pela requisição em um objeto java.
...
DTO (Data Transfer Object) utilizado no retorno das requisições.
```java
package br.com.brunofarias.meubanco.endpoint;
...
- @Id -> Anotação sobre o field definindo que ele sera utilizado como identificado no banco de dados.
- @GeneratedValue -> Anotação sobre o field definindo como o id sera gerado, neste caso, a criterio do banco de dados.
...
Ao extender o CrudRepository o Spring já implementa uma série de metodos úteis como save e findById por exemplo.
```java
package br.com.brunofarias.meubanco.domain.core;
...
Aqui mantemos as regras de negócio do nosso caso de uso.
...
Bom é isso. Ahh! O projetos esta disponível em : gitHub